...
def extract_emails_to_files(max_emails=None):
    ...
    if max_emails:
        logging.info(
            f"Starting email extraction to data/ directory (limit: {max_emails} emails)"
        )
    else:
    ...
    try:
        ...
        for i, email_data in enumerate(emails, 1):
            try:
                ...
                logging.info(
                    f"Processing email {i}/{len(emails)}: {decoded_subject[:50]}..."
                )
                ...
                # Note: email_info structure removed as it was unused
                ...
                email_json = {"email": clean_text, "classification": "typ_1_or_typ_2"}
        ...
        logging.info(
            f"Email extraction completed. Files saved in {data_dir.absolute()}"
        )
        logging.info("Manual steps:")
        logging.info("1. Review files in data/ directory")
        logging.info(
            "2. For spam/ham emails, copy the JSON snippet at the end of each file"
        )
        spam_examples_file = os.getenv("SPAM_EXAMPLES_FILE", "spam_examples.json")
        logging.info(
            f"3. Add to {spam_examples_file} manually (typ 1 = not spam, typ 2 = spam)"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract emails to data/ directory for analysis"
    )
    parser.add_argument(
        "--emails",
        type=int,
